...
在数据模型中我们试图提供一种较为统一的方式去访问各类数据源，而无论它是 SQL、NoSQL、文件或内存，对使用者来说只需要关注数据在应用中的模型即可。

除此之外相关配套的工具也必不可少，我们不希望在减少你一件事情的操作的同时，又带来另外一些额外的操作。
...
- [数据模型](#数据模型) 
    ...
    - [数据访问](#数据访问)
        - [简单、快捷、开箱即用的 ORM 📕](docs/data-model-orm.md) 
        - [数据库](#数据库)
        - [内存](#内存)
        - [文件](#文件)
    - [数据日志](#数据日志)
